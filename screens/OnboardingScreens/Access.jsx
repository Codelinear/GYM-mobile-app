...
const Access = ({navigation}) => {
  ...
  return (
    <View style={tailwind`flex-1 bg-white p-4`}>
      <Text style={tailwind`text-[30px] font-bold mb-4`}>One last thing</Text>
      <Text style={tailwind`text-sm text-black mb-4`}>
        Do you have access to a gym?{' '}
      </Text>
      <Formik
        initialValues={{GymAccess: 'I have access to a gym'}}
            ...
            <View style={tailwind`flex-row flex-wrap gap-4 mb-6`}>
              {goals.map((g, index) => (
                <TouchableOpacity
                  ...
                  style={tailwind`px-4 py-4 border rounded w-full ${
                    values.goal === g
                      ? 'bg-blue-500 border-blue-500'
                      : 'border-gray-300 bg-gray-100'
                  }`}>
